...
The Solr Operator supports using the https://gateway-api.sigs.k8s.io/[Kubernetes Gateway API] for external addressability of SolrClouds.
...
. *Gateway API CRDs*: The Gateway API CRDs must be installed in your cluster
...
. *Gateway Resource*: A Gateway resource must already exist and be managed by your platform team
...
. *Gateway Controller*: A Gateway controller implementation must be running (e.g., NGINX Gateway Fabric, Istio, Envoy Gateway)
...
|*Istio*
|⚠️ Use `DestinationRule` instead

|*NGINX Gateway Fabric*
|✅ Supported

|*GKE Gateway*
|⚠️ Automatic via `appProtocol`
|===
...
* https://gateway-api.sigs.k8s.io/geps/gep-1897/[BackendTLSPolicy (GEP-1897)]
* https://kubernetes.io/docs/concepts/services-networking/service/#application-protocol[Kubernetes Service appProtocol]
